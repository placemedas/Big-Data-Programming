...
def main(inputs, output):
    ...
    # Using collect below as the number of average scores would be few
    out_avg = dict(out_sum.map(average_score).collect())
    # Averages are broadcasted based on below code
    averages = sc.broadcast(out_avg).value

    # For calculating relative score
    commentbysub = comments.map(lambda c: (c['subreddit'],averages.get(c['subreddit'], '-'),c))
    comment_auth = commentbysub.flatMap(relative_score)
